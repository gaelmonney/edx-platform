...
def transcript_download_handler(request, course_key_string):
    ...
    if transcript:
        ...
        filename = filename if filename else '{video_id}_{lang}'.format(video_id=edx_video_id, lang=language_code)
        filename = '{base_name}.srt'.format(base_name=filename.encode('utf8'))
